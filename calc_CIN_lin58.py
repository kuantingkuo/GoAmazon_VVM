...
if __name__ == '__main__':
    ...
    es = np.empty(ds.T.shape)
    for k in range(len(theta_es)):
        es[k] = es_calc(ds.T[k].values)
        qs = 0.622 * es[k] / (pres[k].values - (1 - 0.622) * es[k])
        theta_es[k] = theta_e_calc(pres[k].values, ds.T[k].values, qs)
    e = pres.values*ds.Q.values/(0.622+0.378*ds.Q.values)
#    RH = e[-5]/es[-5]
#    RH=0.85
#    print('RH=',RH*100.)
    q_new = ds.Q.values.copy()
#    q_new[-7:-5] = 0.622*es[-7:-5]*RH/(pres.values[-7:-5]-(1-0.622)*es[-7:-5]*RH)
#    q_new[-7] -= 0.003
#    q_new[-6] -= 0.002
    height = np.empty_like(q_new)
    height[-5] = 685.197
    height[-6] = 921.449
    height[-7] = 1182.769
    height[-8] = 1468.160
    q_new[-7] = 1.353968121111392975e-02
    theta_e[-7] = theta_e_calc(pres[-7].values, ds.T[-7].values, q_new[-7])
    theta_e[-6] = np.interp(height[-6], [height[-5],height[-7]], [theta_e[-5],theta_e[-7]])
    q_new[-6] = Q_from_theta_e(theta_e[-6], pres[-6].values, ds.T[-6].values)
#    for k in range(-6,-8,-1):
#        theta_e[k] = (
#                 (height[-8]-height[k])*theta_e[-5]
#                 + (height[k]-height[-5])*theta_e[-8]
#                ) / (height[-8]-height[-5])
#        q_new[k] = Q_from_theta_e(theta_e[k], pres[k].values, ds.T[k].values)
    print([f"{x:.18e}" for x in q_new])
    Tv = ds.T * (1 + 0.608 * q_new)
    ...
    for k in range(1,len(pres)):
        if (theta_e[k]<theta_es[:k]).all():
            ...
            continue
        print(f'k={k}')
        P_lcl, T_lcl = calc_lcl(theta[k].values, q_new[k])
        for kk in range(k-1,0,-1):
            T_parcel = np.where(pres[kk] < P_lcl,
                                T_parcel_moist(theta_e[k], pres[kk].values, q_new[k]),
