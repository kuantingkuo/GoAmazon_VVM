...
while(it<=tnum)
    ...
    while(i<=2)
        ...
        if(it=1)
            ...
            ymin=subwrd(line,4)
            ymax=subwrd(line,6)
            xpos=xmin+0.3*(xmax-xmin)
            ypos=ymin+0.3*(ymax-ymin)
            'set string 1'
            'set strsiz 0.16 0.2'
            'draw string 'xpos' 'ypos' `3z`1`be`n'
            'draw string 'xpos+0.7' 'ypos+0.1' `3z`1`bes`n'
        endif
        endif
        'off'
        'set cstyle 2'
    ...
    it=it+1
endwhile
'gxprint /data/W.eddie/GoAmazon_VVM_Figs/Fig8_tsels_11_16.png white'
'gxprint /data/W.eddie/GoAmazon_VVM_Figs/Fig8_tsels_11_16.svg white'
